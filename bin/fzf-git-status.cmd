...
set __GIT_HELP=--header "CTRL-A (Select all) / ALT-S (Git add) / ALT-R (Git reset)"
...
set __GIT_ALT_R="alt-s:execute-silent(git reset -- {+2..})+down+reload(%__GIT_CMD%)"

%__GIT_CMD% 2>nul | fzf  %__ZFZ_FRAME% %__ZFZ_LAYOUT% %__ZFZ_PREVIEW_WINDOW%  %__GIT_TITLE% %__GIT_HELP% --nth 2..,.. --multi --preview=%__GIT_PREVIEW_CMD% --bind=%__GIT_ALT_S% --bind=%__GIT_CTRL_A% --bind=%__GIT_ALT_R%
