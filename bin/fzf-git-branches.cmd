...
set __GIT_CMD=git branch --sort=committerdate --sort=HEAD --format="%%(HEAD) %%(color:yellow)%%(refname:short) %%(color:green)(%%(committerdate:relative)) - %%(color:white)%%(subject)%%(color:reset)" --color=always
set __GIT_CMD_ALL=%__GIT_CMD% -a
...
set __GIT_ALT_X="alt-x:execute-silent(git branch -D {1})+reload(%__GIT_CMD:"="""%)"
